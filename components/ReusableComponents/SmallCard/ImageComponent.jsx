...
function ImageComponent({ item }) {
  return (
    <motion.div
      className="relative w-full h-56 bg-gradient-to-br from-gray-100 to-gray-200"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      {item?.image1 && (
        <div className="relative w-full h-full overflow-hidden">
          <Image
            ...
            alt={item?.title || 'عقار'}
            className="object-cover transition-all duration-700 ease-out group-hover:scale-110 group-hover:filter group-hover:brightness-105"
          />
