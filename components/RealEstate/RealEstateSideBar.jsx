...
import RealEstateSideBarMenu from './RealEstateSideBarMenu';
import { TfiMenuAlt } from 'react-icons/tfi';
import { FiActivity } from 'react-icons/fi';
import { useRouter } from 'next/navigation';

export default function RealEstateSideBar({ Button }) {
    ...
    <div className="relative xl:hidden flex justify-between items-center w-full gap-2 bg-three p-2 border-b-[5px] border-one">
      <div>
        ...
        <div className="absolute top-14 lg:top-20 right-0 z-50">
          {isOpen && <RealEstateSideBarMenu setIsOpen={setIsOpen} />}
        </div>
      </div>
      {Button && (
        <button
