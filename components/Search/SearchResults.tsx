...
export default function SearchResults() {
  const { results, loading, error, totalCount, hasMore, loadMore } = useSearch()
        ...
        {results.map((product) => (
          <div key={product.id} className="bg-white rounded-lg shadow-md overflow-hidden">
            <div className="relative h-48">
              {product.image1 ? (
                <Image src={product.image1 || "/placeholder.svg"} alt={product.title} fill className="object-cover" />
              ) : (
